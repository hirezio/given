...
describe('Vitest Given', () => {
  ...
  describe('Then() should pass an empty string to it() given no label', () => {
    beforeEach(() => {
      const itSpy = vi.spyOn(root, 'it').mockImplementation(() => {});
      Then(() => {});
      const actualLabel = itSpy.mock.calls[0][0];
      expect(actualLabel).toBe('');
    });
    it('fake test just to satisfy vitest', () => {});
  });
  ...
  describe('if error is thrown in the "Given" should show a meaningful message', () => {
    ...
    beforeEach(() => {
      vi.spyOn(root, 'beforeEach').mockImplementation((fn: any) => {
  ...
  describe('if an error gets thrown in "When" or "Then"', () => {
    ...
    beforeEach(() => {
      ...
      vi.spyOn(root, 'beforeEach').mockImplementation((fn: any) => fn());

      // queues up afterEach functions for cleanup purposes inside the next "it" function
      vi.spyOn(root, 'afterEach').mockImplementation((fn: any) =>
        afterEachCache.push(fn),
      );
      ...
      vi.spyOn(root, 'it').mockImplementation((desc: any, fn: any) => {
